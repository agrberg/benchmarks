...
| [`array_exclude_or_each_check.rb`](array_exclude_or_each_check.rb) | `Array#-` to exclude an item vs `each` with a `next if` guard | ⚠️ **refined**: the "breaks at 256" claim didn't hold once localized — `next if` still clearly wins through 256; only at 512 does one `Array#-` variant edge past one `next if` variant. "worst position always fastest" still doesn't hold — flips with no stable pattern |
...
| [`array_vs_hash_for_presence.rb`](array_vs_hash_for_presence.rb) | `Array#include?`/`index`/`bsearch` vs `Hash[]`/`Hash#key?` for membership | ⚠️ **refined**: `Hash[]` wins from 8 through 1000; `Hash#key?` overtakes it at 10000 (new — the old sparse range never tested that far past 1000 densely enough to see it). ⚠️ Size 1 specifically is unreproducible — the benchmark's own setup uses unseeded `rand`, so don't trust whichever approach "wins" there |
...
| [`hash_or_assign.rb`](hash_or_assign.rb) | `hash[k]=` vs `hash[k] ||=` vs a default-proc `Hash` | **(reversed, ✅ confirmed and localized)** `[]=` wins through 8 items (tied with `||=` only at 1); `||=` takes over precisely at 16 and stays ahead; `[]=` becomes outright slowest from 1000+; `default` wins from 100+ |
| [`hash_or_json.rb`](hash_or_json.rb) | Recursive hash-key regex search vs `to_json` + regex index search | ⚠️ **refined**: recursive scan wins in most cases; for the one case where `to_json` wins (early nested match), it takes over starting at size 10, not 100 as first read — that first read predated true-benchmark-ips-defaults verification |
...
| [`hash_with_obj_or_array_to_h.rb`](hash_with_obj_or_array_to_h.rb) | `each_with_object({})` vs `map`+`Array#to_h` for transforming a hash | ⚠️ **refined, extended to 1000**: a real three-regime pattern, not pure noise — `each_with_object` wins small (1/4/8), `Array#to_h` takes over mid-range (16–100), `each_with_object` wins again at 1000. Size 2 alone keeps flipping between runs |
...
| Benchmark | Compares | Finding |
|---|---|---|
| [`base64.rb`](base64.rb) | `String#unpack('m*')` vs `Base64.decode64` | **(reversed, refined and localized)** tied with `unpack` at 1 — this flips between "tied" and "decode64 slightly ahead" run to run, closer to the *original* pre-reversal finding; `decode64` clearly wins 4 through 1000; `unpack` flips back ahead at 10000 |
...
| [`string_shovel_join_or_interp.rb`](string_shovel_join_or_interp.rb) | Interpolation vs `Array#join` vs `<<` for concatenation | ⚠️ **refined**: a `j=50` grid point isolated the real driver — it's the *appended* string's absolute length (j), not the base/appended ratio. `<<` wins outright whenever j=5 (any base length); interpolation wins outright whenever j≥50 (any base length); j=20 is a genuine toss-up (noise, not a real sub-boundary). `interp + <<` never wins outright; `join` stays consistently slowest |
...
| Benchmark | Compares | Finding |
|---|---|---|
| [`attribute_sum_benchmark.rb`](attribute_sum_benchmark.rb) | Manual loop, `inject`, `collect.sum`, `sum(&:value)`, dual-accumulator loop | ✅ Switched `TIMES` from a contiguous `2..16` (needed a 240s timeout) to a sparse `[2, 16, 100, 1_000, 10_000]` — ~75% cheaper *and* confirms a cleaner, more stable finding: `sum(&:value)` wins at every size (tied with manual only at 2); ranking from 16 on is stable: `sum(&:value)` > `collect.sum` > manual > `inject` > "manual 2x" (reversed from historical — now consistently slowest) |
...
| [`with_index_reduce_chain.rb`](with_index_reduce_chain.rb) | `each_with_index.reduce` vs `each_with_object.with_index` for building an index=>value hash | ⚠️ **noise, corrected and extended to 4096**: genuinely noise-equivalent, not a findable crossover — the margin keeps shrinking (0.25% by 4096) rather than diverging, and which one's ahead at a given size changes between repeated runs |
...
- **9 files had their `TIMES` sweep densified** after review, to actually localize crossovers the original coarse ranges only hinted at (`array_exclude_or_each_check.rb`, `array_vs_hash_for_presence.rb`, `hash_or_assign.rb`, `base64.rb`, `hash_or_json.rb`, `string_shovel_join_or_interp.rb`, `with_index_reduce_chain.rb`, `hash_with_obj_or_array_to_h.rb`) or sparsified to cut wasted runtime (`attribute_sum_benchmark.rb`, whose old contiguous `2..16` range needed a 240s timeout for no added signal). Re-run at true benchmark-ips defaults, this pass pinned several crossovers precisely (e.g. `hash_or_assign.rb`'s flip is exactly between 8 and 16 items) and reversed a couple of the earlier localization guesses themselves — `array_exclude_or_each_check.rb`'s "breaks at 256" didn't hold once tested at 128/256/512, and `array_vs_hash_for_presence.rb` surfaced an actual benchmark-design flaw (unseeded `rand` in its own setup makes the size-1 case non-reproducible across runs, unrelated to Ruby version). `template.rb`'s default `TIMES` was updated to `[1, 4, 16, 100, 1_000, 10_000]` accordingly — see [`CLAUDE.md`](CLAUDE.md) for the runtime-cost model behind that choice.
