...
- When input size matters, sweep it rather than picking one N — the common pattern is `TIMES = [1, 16, 100, 1_000, 10_000]`, looping `TIMES.each` inside the lambda and suffixing report labels with the size (e.g. `"#{i} - Array#include?"`). This is how crossover points get found (e.g. `set_or_array_lookup.rb` shows Array beats Set below 4 elements, loses above it).
